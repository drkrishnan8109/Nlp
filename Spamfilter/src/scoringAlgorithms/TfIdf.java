...
public class TfIdf {
	...
	private void calculateTermFrequency() {
		for (Document document : dataSet) {
			...
			String[] allTokens = document.getAllToken();
	...
	private void calculateDocumentFrequency() {
		...
		for (String token : uniqueTokens) {
			...
			Double value = 0.0;
			try {
				value = Math.log10(totalNumberOfDocuments / totalTokenFrequency);
			} catch (Exception e) {
		...
		for (String key : df.keySet()) {
			System.out.println("Terms :" + key + "  " + df.get(key).toString()
					+ "\n");
		}
		for (String key : idf.keySet()) {
			System.out.println("Terms :" + key + "  " + idf.get(key).toString()
					+ "\n");
		}
		System.out.println("Count of Total terms :" + idf.size());
	}
	...
	private Integer getTokenFrequency(String token) {
		...
		for (Document document : dataSet) {
			if (document.hasToken(token)) {
				totalTokenFrequency = totalTokenFrequency
						+ 1;
			}
		}
	...
	private void calculateUnnormalizedTfIdf() {
		// tfidf (t,d,D) = tf(t,d) * idf (t,D)
		for (Document document : dataSet) {
			HashMap<String, Double> allTokenTfIdf = new HashMap<String, Double>();
			for (String token : document.getAllToken()) {
				allTokenTfIdf.put(token, document.getTokenFrequency(token)
						* idf.get(token));
			}
	...
	public void prepareDataInputForWeka() {
		File arffFile = new File("tfidf.arff");
		try {
